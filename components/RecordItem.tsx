...
const RecordItem = ({ record }: { record: Record }) => {
  ...
  const handleDeleteRecord = async (recordId: string) => {
    console.log('RecordItem - Starting delete for recordId:', recordId);
    console.log('RecordItem - Record data:', record);
    setIsLoading(true); // Show loading spinner
    setError(null); // Clear any previous errors
    try {
      const result = await deleteRecord(recordId); // Perform delete operation
      console.log('RecordItem - Delete result:', result);
      if (result.error) {
        ...
        console.log('Record deleted successfully:', result.message);
  ...
  return (
    ...
    >
      ...
      {error && (
        <div className="absolute top-0 left-0 right-0 bg-red-500 text-white text-xs p-2 rounded-t-xl z-10">
